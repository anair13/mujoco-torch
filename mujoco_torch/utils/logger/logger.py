"""
Copied from Vitchyr's rlkit and merged features with the baselines logger
...
from mujoco_torch.utils.logger.tabulate import tabulate
...
def debug(*args):
    do_log(*args, level=DEBUG)

def info(*args):
    do_log(*args, level=INFO)

def warn(*args):
    do_log(*args, level=WARN)

def error(*args):
    do_log(*args, level=ERROR)
